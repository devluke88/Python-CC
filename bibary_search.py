# Google Python Coursera - module 4 Troubleshooting and Debugging
...
def find_item(list, item):
  ...
  if len(sorted(list)) == 0:
    return False

  #Is the item in the center of the list?
  middle = (len(list)-1)//2
  if sorted(list)[middle] == item:
    return True

  #Is the item in the first half of the list? 
  if item < sorted(list)[middle]:
    #Call the function with the first half of the list
    return find_item(sorted(list)[:middle], item)
  else:
    #Call the function with the second half of the list
    return find_item(sorted(list)[middle+1:], item)
...
# 2. The binary_search function returns the position of key in the list if found, or -1 if not found. We want to make sure that it's working correctly, so we need to place debugging lines to let us know each time that the list is cut in half, whether we're on the left or the right. Nothing needs to be printed when the key has been located.
